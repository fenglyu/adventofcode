...
/*
伪代码核心逻辑说明
预处理层：通过旋转 / 翻转生成每个 Tile 的 8 种朝向，并提取边框（转为字符串便于比较），解决 “相机故障导致朝向随机” 的问题。
图构建层：通过对比所有 Tile 的边框，确定相邻关系并构建邻接图，将 “拼接匹配” 转化为 “图的边”。
遍历定位层：以角落块（度 = 2）为起点，用 BFS 遍历图，通过「边框匹配→确定朝向」「方向推导→确定坐标」两个核心步骤，给每个 Tile 分配空间位置。
拼接层：按坐标排列 Tile，去除边框后合并像素，得到完整图像。
*/